...
public class Day22 extends AbstractDay {
	...
	*/
	 ...
	 * Given depth.
	 */
	private static final int DEPTH = 11817;
	
	/**
	 * Given target.
	 */
	private static final Region TARGET = new Region(9, 751, DEPTH, 0);
	...
	/**
	 * Array of regions that form the cavern.
	 */
	private Region[][] cavern = new Region[WIDTH][HEIGHT];
	
	/**
	 * Constructor for Day 22 puzzles.
	 */
	public Day22() {
		this.dayNum = "22";
	}
	...
	public void puzzleOne() {
		...
		In the cave system above, because the mouth is at 0,0 and the target is
		at 10,10, adding up the risk level of all regions with an X coordinate
		...
		*/
		...
		this.cavern[0][0] = new Region(0, 0, DEPTH, 0);
		
		for(int x=1; x<WIDTH; x++){
			this.cavern[x][0] = new Region(x, 0, DEPTH, x * Region.X_COEFFICIENT);
		}
		
		for(int y=1; y<HEIGHT; y++){
			this.cavern[0][y] = new Region(0, y, DEPTH, y * Region.Y_COEFFICIENT);
		}
		
		for(int x=1; x<WIDTH; x++){
			for(int y=1; y<HEIGHT; y++){
				final int geologicIndex = this.cavern[x-1][y].getErosionLevel() *
					this.cavern[x][y-1].getErosionLevel();
				this.cavern[x][y] = new Region(x, y, DEPTH, geologicIndex);
			}
		}
		
		this.cavern[9][751] = TARGET;
		
		int totalRisk = 0;
		for(int x=0; x<WIDTH; x++){
			for(int y=0; y<HEIGHT; y++){
				totalRisk += this.cavern[x][y].getRisk();
			}
		}
		
		LOGGER.debug("Day 22, Puzzle 1: risk is " + totalRisk);
		this.solutionOne = String.valueOf(totalRisk);
	}
	...
	public void puzzleTwo() {
		...
		might involve entering regions beyond the X or Y coordinate of the
		target.
		...
		region. Moving to an adjacent region takes one minute. (For example, if
		you have the torch equipped, you can move between rocky and narrow
		...
		you can find him in the dark. The target is always in a rocky region, so
		if you arrive there with climbing gear equipped, you will need to spend
