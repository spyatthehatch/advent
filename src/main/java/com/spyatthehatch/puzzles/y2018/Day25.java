...
public class Day25 extends AbstractDay {
	...
	bearded man have a solution. At least the reindeer has a warm place to rest:
	...
	pick some up; just don't forget a mug! Is there anything else I can help you
	with today?"
	...
	*/
	...
	public void puzzleOne() {
		...
		manhattan distance no more than 3 from the last, between the two of them.
		(That is, if a point is close enough to a constellation, it "joins" that
		...
		0,0,0,6 is connected to the others by being 3 away from 0,0,0,3, which is
		...
		number of constellations is 2. (If a point at 6,0,0,0 were present, it
		would connect 3,0,0,0 and 9,0,0,0, merging all of the points into a
		...
		*/
		...
		final List<Constellation> constellations = new
			ArrayList<Constellation>();
		
		for(final FourDPoint point : this.points){
			if(constellations.isEmpty()){
				constellations.add(new Constellation(point));
			} else {
				List<Constellation> combined = new ArrayList<Constellation>();
				final Constellation newGroup = new Constellation(point);
				
				for(final Constellation c : constellations){
					if(c.addPoint(point)){
						combined.add(c);
					}
				}
				
				for(final Constellation c : combined){
					newGroup.join(c);
					constellations.remove(c);
				}
				
				constellations.add(newGroup);
			}
		}
		
		LOGGER.debug("Day 25, Puzzle 1: there are " + constellations.size() +
			" constellations.");
		this.solutionOne = String.valueOf(constellations.size());
	}
	...
	public void puzzleTwo() {
		...
		you set up a few days ago. You check the status of the device:
