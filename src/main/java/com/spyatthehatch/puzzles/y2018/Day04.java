...
public class Day04 extends AbstractDay {
	...
	*/
	 ...
	 * Map of Guards, by Id.
	 */
	public final Map<Integer, Guard> guards = new HashMap<Integer, Guard>();
	...
	public Day04() {
		...
		for(final Event e : this.events) {
			if(e.getAction() == Action.START) {
				guardId = e.getGuardId();
			} else {
				e.setGuardId(guardId);
			}
		}
	}
		...
		for(final Event e : this.events) {		
			switch(e.getAction()) {
				...
				case ASLEEP:
					break;
				case AWAKE:
					Guard g = this.guards.get(e.getGuardId());
					
					if (g == null) {
						g = new Guard(e.getGuardId());
						this.guards.put(g.getId(), g);
					}
					...
					break;
				default:
					LOGGER.error("Encountered an event with an unknown state.");
			}
			
			lastEvent = e;
		}
		
		LOGGER.trace("guards map has " + this.guards.size());
		
		Guard sleepyGuard = null;
		for(Map.Entry<Integer, Guard> entry : this.guards.entrySet()) {
			if(sleepyGuard == null) {
				sleepyGuard = entry.getValue();
			} else {
				if(entry.getValue().getMinutesAsleep() >
					sleepyGuard.getMinutesAsleep()) {
					sleepyGuard = entry.getValue();
				}
			}
		}
	...
	public void puzzleTwo() {
		...
		*/
		...
		for(final Map.Entry<Integer, Guard> entry : this.guards.entrySet()) {
			final Guard g = entry.getValue();
			final int sleepyMinute = g.getSleepiestMinute();
			final int days = g.getMinuteValue(sleepyMinute);
			...
			if(sleepyGuard == null) {
				...
				LOGGER.trace("Guard " + g.getId() + " is sleepy at " + sleepyMinute
					+ " on " + days + " days.");
