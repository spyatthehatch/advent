...
public class Day02 extends AbstractDay {
	...
	*/
	 ...
	 * Puzzle resource.
	 */
	public static final String RESOURCE = "puzzles/2019/day02-list.txt";
	
	/**
	 * Array of original integers.
	 */
	 ...
	 */
	public Day02() {
		LOGGER.debug("Day 2: 1202 Program Alarm");
		this.dayNum = "2";
	...
	public void puzzleOne() {
		...
		*/		
		final int output = processScript(12, 2);
	 ...
	 */
	public int processScript(final int noun, final int verb){
		int[] program = new int[this.origArray.length];
		System.arraycopy(this.origArray, 0, program, 0, this.origArray.length);
		...
		for(int ip = 0; ip < program.length; ip += 4){
			if(processInstruction(program, ip)){
				continue;
			} else {
				break;
			}
	 ...
	 * 
	 * @param program Array holding instructions and parameters.
	 * @param ip Instruction pointer.
	 * @return true if program should continue.  False, otherwise.
	 */
	public boolean processInstruction(int[] program, final int ip){
		final int left = program[ip + 1];
		final int right = program[ip + 2];
		final int store = program[ip + 3];
		
		if(program[ip] == ADD){
			program[store] = program[left] + program[right];
			return true;
		} else if (program[ip] == MULTIPLY) {
			program[store] = program[left] * program[right];
			return true;
		} else if (program[ip] == HALT){
			return false;
		} else {
			LOGGER.warn("Invalid program command encountered.");
			return false;
		}
	}
}